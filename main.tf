...
resource "github_repository_file" "module_files" {
  ...
  branch              = var.branch
  file                = "${var.module_path}/${local.module_files[count.index]}"
...
resource "github_repository_file" "sample_files" {
  ...
  repository = var.repository
  branch     = var.branch
  file       = "examples/${var.module_path}/${local.sample_files[count.index]}"
  content = templatefile("${path.module}/files/examples/${local.sample_files[count.index]}",
    {
